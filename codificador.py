...
class Codificador():
    ...
    def codificar(self, arquivo, tipo, golomb_divisor = 0):
        ...
        if arquivo.rfind('.') >= 0:
            self.arquivo_destino = open(''.join([arquivo[:arquivo.rfind('.')], extensao]), 'wb')
        else:
            self.arquivo_destino = open(''.join([arquivo, extensao]), 'wb')
    ...
    ##### PARA TESTE #####
    # Envia os mesmos valores de entrada, mas em formato bit (para teste)
    def _retornar_igual(self, valor):
        return format(valor, 'b').zfill(8)